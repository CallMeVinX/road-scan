...
export function StageConvolution({ uploadedImage }: StageConvolutionProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSimStep(prev => (prev + 1) % (totalSteps + 2))
    }, 1200)
  ...
  return (
    <div className="grid gap-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:p-7 shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="group relative rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
            ...
            <div className="relative h-64 lg:h-72 flex items-center justify-center p-2">
              {uploadedImage ? (
          ...
          {/* Result Image - Light Theme */}
          <div className="relative rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
            <div className="absolute top-3 left-3 z-10 bg-blue-50 text-blue-700 border border-blue-200 text-[10px] font-bold tracking-wider px-3 py-1.5 rounded-md shadow-sm">
              HASIL KONVOLUSI
            </div>
            <div className="relative h-64 lg:h-72 flex items-center justify-center p-2">
              {isLoading ? (
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-blue-600 font-medium text-sm animate-pulse">Menghitung fitur tepi...</p>
                </div>
              ) : resultImage ? (
                <img src={resultImage} alt="Edge Detection Result" className="h-full w-full object-contain rounded-lg" />
              ) : (
                <span className="text-slate-500 text-sm italic">Klik tombol untuk memulai proses</span>
              )}
            </div>
          </div>
        </div>
      ...
      {/* --- BAGIAN 2: SIMULASI MATEMATIKA LIVE (Light Theme) --- */}
      {resultImage && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:p-7 shadow-sm">
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-center justify-center overflow-x-auto pb-4">
            ...
            <div className="flex flex-col items-center shrink-0">
              ...
              <div className="grid gap-1 p-3 bg-white border border-slate-200 rounded-xl shadow-sm" style={{ gridTemplateColumns: 'repeat(6, 2.5rem)' }}>
                    ...
                    return (
                      <div key={`in-${rIdx}-${cIdx}`}
                        className={`h-10 w-10 flex items-center justify-center text-[11px] font-mono rounded transition-all duration-300 ${
                          isActive 
                            ? 'bg-blue-600 text-white font-bold scale-110 shadow-md z-10' 
            ...
            <div className="flex flex-col items-center shrink-0">
              ...
              <div className="grid gap-1 p-3 bg-white border border-slate-200 rounded-xl shadow-sm" style={{ gridTemplateColumns: 'repeat(3, 2.5rem)' }}>
                {visualKernel.map((row, rIdx) => 
                  row.map((val, cIdx) => (
                    <div key={`k-${rIdx}-${cIdx}`}
                      className={`h-10 w-10 flex items-center justify-center text-xs font-bold rounded border ${
                        val === 0 ? 'bg-slate-50 text-slate-400 border-slate-100' : 
                        val > 0 ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-red-50 text-red-500 border-red-200'
                      }`}
                    >
                      {val}
                    </div>
            ...
            <div className="flex flex-col items-center shrink-0">
              ...
              <div className="grid gap-1 p-3 bg-white border border-slate-200 rounded-xl shadow-sm" style={{ gridTemplateColumns: 'repeat(4, 2.5rem)' }}>
                  ...
                  Array.from({ length: outputSize }).map((_, cIdx) => {
                    ...
                    return (
                      <div key={`out-${rIdx}-${cIdx}`}
                        className={`h-10 w-10 flex items-center justify-center text-[11px] font-mono rounded transition-all duration-300 ${
                          isActive ? 'bg-green-500 text-white font-bold scale-110 shadow-md' : 
                          isDone ? 'bg-green-50 text-green-700 font-bold border border-green-200' : 
                          'bg-slate-50 text-transparent border border-dashed border-slate-200'
                        }`}
                      >
                        {isDone || isActive ? 'OK' : ''}
                      </div>
          ...
          <div className="mt-6 mx-auto max-w-4xl bg-slate-50 rounded-xl p-4 lg:p-5 border border-slate-200 font-mono text-[11px] sm:text-xs">
            <div className="flex justify-between items-center mb-3 border-b border-slate-200 pb-2">
              <span className="text-slate-600 font-semibold">Terminal Perhitungan</span>
              <span className="text-blue-600 font-bold bg-blue-100 px-2 py-1 rounded">Step: {isCalculating ? simStep + 1 : 'Selesai'}</span>
            </div>
            
            {isCalculating && liveCalculation ? (
              <div className="space-y-2 text-slate-700">
                <p>
                  <span className="text-indigo-600 font-semibold">Σ (Piksel × Kernel)</span> = {liveCalculation.formula}
                </p>
                <p>
                  <span className="text-amber-600 font-semibold">Nilai Mentah</span> = {liveCalculation.rawSum}
                </p>
                <p className="pt-2 border-t border-slate-200">
                  <span className="text-green-600 font-bold">Intensitas Piksel Baru (Absolut 0-255) = {liveCalculation.finalVal}</span>
                </p>
              </div>
            ) : (
              <p className="text-slate-500 italic flex items-center h-full min-h-[4rem]">
                <span className="w-2 h-4 bg-slate-400 animate-pulse mr-2 inline-block"></span>
